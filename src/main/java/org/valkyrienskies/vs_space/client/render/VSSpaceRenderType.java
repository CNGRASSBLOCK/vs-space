...
public class VSSpaceRenderType extends RenderType {
    ...
    private static final Function<ResourceLocation, RenderType> PlanetRender = Util.memoize((p_286149_) -> create("planet_render", DefaultVertexFormat.NEW_ENTITY, VertexFormat.Mode.QUADS, 256, true, true, CompositeState.builder().setShaderState(new ShaderStateShard(VSSpaceShader.PlanetRender)).setTextureState(new RenderStateShard.TextureStateShard(p_286149_, false, false)).setTransparencyState(TRANSLUCENT_TRANSPARENCY).setLightmapState(LIGHTMAP).createCompositeState(false)));
